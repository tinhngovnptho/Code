...
void update(int id, int l, int r, int u, int v, int val) {
	if (l > v || r < u) return ; 
	if (u <= l && r <= v) {
		st[id].first = add(st[id].first, mul(r - l + 1, val))
		;
...
void Main() {
	...
	for (int i = 1; i <= n; ++i) update(1, 1, n, i, i, fixmod(a[i]));
	...
	while (q--) {
		...
		if (type == 1) {
			...
			// for (int i = 1; i <= n; ++i) cout << query1(1, 1, n, i, i) << ' ';
			// cout << '\n';
		} else {
			// cout << query1(1, 1, n, l, r) << '\n';
			cout << sub(query2(1, 1, n, l, r), mul(l - 1, query1(1, 1, n, l, r)))  << '\n';
		}
	}
}
