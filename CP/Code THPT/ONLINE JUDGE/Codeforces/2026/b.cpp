...
void process(void) {
	...
	if (n % 2 == 0) {
		int64_t res = -1; 
		for (int i = 2; i <= n; i += 2) {
			maximize(res, a[i] - a[i - 1]);
		}
		cout << res << '\n'; 
		return ;
	}
	int64_t ans = INF, idx = -1; 
	for (int i = 1; i <= n; i += 2) {
		int64_t res = 1;
		for (int j = 2; j <= i - 1; j += 2) maximize(res, a[j] - a[j - 1]);
		for (int j = i + 2; j <= n; j += 2) maximize(res, a[j] - a[j - 1]); 
		if(minimize(ans, res)) idx = i;
	}
	cout << ans << '\n';
}
